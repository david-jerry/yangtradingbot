...
from constants import help_message, about_message, terms_message, language_message, welcome_message, wallets_message
...
terms_keyboard = [
    [accept_terms_button, decline_terms_button]
]
...
wallets_assets = InlineKeyboardButton("Wallet Assets", callback_data="start_wallet_assets")
...
token_transfer = InlineKeyboardButton("Token Transfer", callback_data="start_token_transfer")
...
async def terms_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        if button_data == "accept":
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"<pre>You have accepted our terms and condition</pre>", parse_mode=ParseMode.HTML)
            await query.edit_message_text(text=welcome_message, parse_mode=ParseMode.HTML, reply_markup=start_button_mu)
        elif button_data == "decline" and status != "accept":
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"<pre>You have accepted our terms and condition</pre>", parse_mode=ParseMode.HTML)
            await query.edit_message_text(text=welcome_message, parse_mode=ParseMode.HTML, reply_markup=start_button_mu)
        else:
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"<pre>You previously accepted our terms and condition, please contact the helpdesk: https://t.me/RDTradinghelpdesk @RDTradinghelpdesk for assistance.</pre>", parse_mode=ParseMode.HTML)
    else:
        await query.message.reply_photo(bot_profile_photo, caption=language_message, parse_mode=ParseMode.HTML, reply_markup=language_markup)
...
async def language_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        if button_data == "en":
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"Language Selected: <pre>{button_data.lower()}</pre>", parse_mode=ParseMode.HTML)
        elif button_data == "de":
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"Language Selected: <pre>{button_data.lower()}</pre>", parse_mode=ParseMode.HTML)
        elif button_data == "it":
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"Language Selected: <pre>{button_data.lower()}</pre>", parse_mode=ParseMode.HTML)
    ...
    if match:
        ...
        if button_data == "about":
            if bot_profile_photo:
                message = await query.message.reply_photo(bot_profile_photo, caption=about_message, parse_mode=ParseMode.HTML, reply_markup=home_markup)
            else:
                message = await query.message.reply_text(about_message, parse_mode=ParseMode.HTML, reply_markup=home_markup)
        elif button_data == "terms":
            if bot_profile_photo:
                message = await query.message.reply_photo(bot_profile_photo, caption=terms_message, parse_mode=ParseMode.HTML, reply_markup=terms_markup)
            else:
                message = await query.message.reply_text(terms_message, parse_mode=ParseMode.HTML, reply_markup=terms_markup)
        elif button_data == "language":
            if bot_profile_photo:
                message = await query.message.reply_photo(bot_profile_photo, caption=language_message, parse_mode=ParseMode.HTML, reply_markup=language_markup)
            else:
                message = await query.message.reply_text(language_message, parse_mode=ParseMode.HTML, reply_markup=language_markup)
        elif button_data == "wallets":
            message = await query.message.reply_text(wallets_message, parse_mode=ParseMode.HTML, reply_markup=chain_markup)
            if "message_stack" not in context.user_data:
                context.user_data["message_stack"] = []
            context.user_data["message_stack"].append({'message': message, 'text':wallets_message, 'markup':chain_markup})
        elif button_data == "help":
            if bot_profile_photo:
                message = await query.message.reply_photo(bot_profile_photo, caption=help_message, parse_mode=ParseMode.HTML, reply_markup=home_markup)
            else:
                message = await query.message.reply_text(help_message, parse_mode=ParseMode.HTML, reply_markup=home_markup)
...
async def wallets_chain_connect_button_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    await query.answer()
    command = query.data
    ...
    context.user_data["message_stack"].append({'message': context_message, 'text':context_text, 'markup':context_markup})
    status = user_data[user_id]['agreed_to_terms']
    
    if status != 'accept':
        message = await query.edit_message_text(text=terms_message, parse_mode=ParseMode.HTML, reply_markup=home_markup)
        return message
    ...
    if match:
        ...
        if button_data == "create":
            ...
            wallet_address, private_key, balance, mnemonic = generate_wallet(NETWORK)
...
⚠ Make sure to save this mnemonic phrase OR private key using pen and paper only. Do NOT copy-paste it anywhere if not certain of the security. You could also import it to your Metamask/Trust Wallet. After you finish saving/importing the wallet credentials, delete this message. The bot will not display this information again.
            ...
            await query.edit_message_text(text=message, parse_mode=ParseMode.HTML, reply_markup=home_markup)
...
async def home_button_callback(update: Update, context: CallbackContext):
    ...
    if bot_profile_photo:
        await query.message.reply_photo(bot_profile_photo, caption=welcome_message, parse_mode=ParseMode.HTML, reply_markup=start_button_markup)
    else:
...
async def back_button_callback(update: Update, context: CallbackContext):
    ...
    if previous_messages:
        ...
        if last_message.get('markup') is not None:
            await query.edit_message_text(text=last_message['text'], parse_mode=ParseMode.HTML, reply_markup=last_message['markup'])
        else:
            await query.edit_message_text(text=last_message['text'], parse_mode=ParseMode.HTML)
