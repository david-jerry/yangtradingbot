...
async def log_loop(event_filter, poll_interval):
    ...
    while True:
        try:
            ...
            token_address = web3.to_checksum_address("0x00") 
            #This line will give bug "An unexpected error occurred: Unknown format '0x00', attempted to normalize to '0x00'"
            pair = uniswap_factory.functions.getPair(token_address, weth).call()
            response = requests.get(f"{DEXSCREENER_ENDPOINT}{pair}")
            print(response.json())
            if response.status_code == 404:
                print(f"Pair not found for {token_address}")
            elif response.status_code == 500:
                print(f"Internal server error for {token_address}")
            else:
                response.raise_for_status()  # Raise other HTTP-related exceptions
