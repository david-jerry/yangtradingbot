...
async def checkDatabase(bot_token,contract_ab):
    ...
    currentcopytradetxhash = copytradetxhash =  await Load_trade_txhash_copy_data()
    uniswap_router = web3.to_checksum_address(UNISWAP_ROUTER)
    weth = web3.to_checksum_address(WETH)
    uniswap = web3.eth.contract(address=uniswap_router, abi=UNISWAP_ABI)
    while True:      
          copytradetxhash = await Load_trade_txhash_copy_data()
          if len(currentcopytradetxhash) < len(copytradetxhash):
               for i in range(len(currentcopytradetxhash), len(copytradetxhash)):
                    ...
                    if (name == 'Wrapped Ether'):
                        name = 'Ether'
                    amount_message = copytradetxhash[i]['amount']
                    fullmessage = f'Trade token: Swap to {amount_message} ${symbol} ${name} \n Transaction hash : {txhash}'
